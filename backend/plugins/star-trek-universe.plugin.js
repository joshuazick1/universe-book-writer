...
export default class StarTrekUniversePlugin {
    ...
    validation = {
        ...
        async validateLocation(location) {
            ...
            const loc = location;
            // Validate required fields
            if (!loc.sector || !loc.system || !loc.classification)
                return false;
    ...
    validateContent(content) {
        ...
        if (contentObj.characters && Array.isArray(contentObj.characters)) {
            for (const character of contentObj.characters) {
                const char = character;
                if (!this.isValidSpecies(char.species)) {
                ...
                if (char.rank && !this.isValidStarfleetRank(char.rank)) {
        ...
        if (contentObj.technology && Array.isArray(contentObj.technology)) {
            for (const tech of contentObj.technology) {
                const techObj = tech;
                if (techObj.warpFactor && techObj.warpFactor > 9.9) {
     ...
     */
    getAIPrompts(_context) {
        const prompts = [];
        // Add base Star Trek context
        prompts.push('You are writing in the Star Trek universe. Maintain consistency with established canon, ' +
