...
export class PluginValidator {
        ...
        try {
            ...
            if (typeof testPlugin.initialize === 'function') {
                await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Initialization timeout')), this.config.performance.maxInitializationTime)),
                ]);
            }
    ...
    generateReport(pluginName, results) {
        ...
        const performanceResults = results.filter(r => r.rule.includes('performance') ||
