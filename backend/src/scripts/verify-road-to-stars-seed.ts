...
export async function verifyRoadToStarsSeeding(): Promise<void> {
  ...
  try {
    ...
    results.push(
      await verifyCollection(db, 'universes', 1, 'Should have United Republic of Planets universe')
    );
    ...
    for (const result of results) {
      ...
      console.log(
        `${status} ${result.collection}: ${result.actual}/${result.expected} ${result.passed ? 'PASSED' : 'FAILED'}`
      );
