...
export async function createTestApp(testDb, mongoClient) {
    ...
    const authUseCase = new AuthUseCase(userRepository, authTokenRepository, authSessionRepository, passwordService, tokenService, emailService, securityService);
    // Create a mock AdminSettingsRepository for testing
    ...
    const adminUseCase = new AdminUseCase(userRepository, adminSettingsRepository, securityService, passwordService, emailService);
    ...
    const authMiddleware = new AuthMiddleware(tokenService, authTokenRepository, authSessionRepository, userRepository);
 ...
 */
let globalSecurityService = null;
/**
...
export function resetRateLimiting() {
    if (globalSecurityService) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        globalSecurityService.rateLimitStore.clear();
    }
 ...
 */
export function clearRateLimitFor(identifier) {
    if (globalSecurityService) {
