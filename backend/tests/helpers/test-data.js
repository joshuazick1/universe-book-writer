...
export async function createTestUser(db, options = {}) {
    const { email = 'test@example.com', password = 'SecureTestP@ssw0rd!', role = UserRole.USER, status = UserStatus.ACTIVE, // Default to ACTIVE for tests
    emailVerified = true, firstName = 'Test', lastName = 'User', } = options;
...
export async function createTestSession(db, userId, options = {}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
) {
    const { refreshTokenId = 'refresh-token-' + Date.now(), deviceInfo = {
        userAgent: 'Mozilla/5.0 (Test Browser)',
        ip: '127.0.0.1',
        deviceId: 'test-device-123',
    }, expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // 7 days
    isActive = true, } = options;
...
export async function createTestToken(db, userId, type, options = {}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
) {
    const { token = `test-${type.toLowerCase()}-token-${Date.now()}`, expiresAt = new Date(Date.now() + (type === 'ACCESS' ? 15 * 60 * 1000 : 7 * 24 * 60 * 60 * 1000)), isRevoked = false, usedAt = null, } = options;
...
export async function createTestSecurityLog(db, userId, eventType, options = {}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function createTestAdminUser(db) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function createTestRegularUser(db) {
    return createTestUser(db, {
        email: 'user@test.com',
...
export async function createExpiredToken(db, userId, type
// eslint-disable-next-line @typescript-eslint/no-explicit-any
) {
...
export async function createRevokedToken(db, userId, type
// eslint-disable-next-line @typescript-eslint/no-explicit-any
) {
    return createTestToken(db, userId, type, {
        isRevoked: true,
...
export async function createSuspiciousSecurityLogs(db, userId, count = 5
// eslint-disable-next-line @typescript-eslint/no-explicit-any
) {
    const logs = [];
...
export async function waitForDatabaseWrite(ms = 100) {
