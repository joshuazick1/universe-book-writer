...
const Dashboard: React.FC = () => {
  ...
  const refreshBooks = async (universeId: string) => {
    ...
    const res = await fetch(`/api/books?universeId=${encodeURIComponent(universeId)}`);
    const data = await res.json();
    setBooks(data);
  };
  ...
  const handleUpdateUniverse = async () => {
    ...
    await fetch(`/api/universes/${editUniverseId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleSendAiMessage = async () => {
    ...
    setTimeout(() => {
      setAiMessages(msgs => [...msgs, { role: 'ai', content: '(AI response placeholder)' }]);
    }, 800);
  };
  ...
  return (
    <div className="bg-white rounded shadow p-6 max-w-4xl mx-auto">
      ...
      <div className="flex">
        <aside className="w-64 mr-6 bg-gray-50 border-r p-4 rounded h-full">
          <h3 className="font-bold mb-2">AI Helper</h3>
          <div className="mb-4">
            <button
              className="bg-purple-600 text-white px-3 py-1 rounded mb-2 w-full"
              onClick={() => setAiInput('Can you give me worldbuilding advice?')}
            >
              Ask for Worldbuilding Advice
            </button>
            <button
              className="bg-purple-600 text-white px-3 py-1 rounded mb-2 w-full"
              onClick={() => setAiInput('Suggest some universe rules.')}
            >
              Suggest Universe Rules
            </button>
            <button
              className="bg-purple-600 text-white px-3 py-1 rounded mb-2 w-full"
              onClick={() => setAiInput('Generate some lore for my universe.')}
            >
              Generate Lore
            </button>
          </div>
          <div className="border rounded bg-white p-2 h-64 overflow-y-auto mb-2">
            {aiMessages.length === 0 && (
          ...
          >
            ...
            />
            <button className="bg-purple-600 text-white px-3 py-1 rounded" type="submit">
              Send
            </button>
          </form>
