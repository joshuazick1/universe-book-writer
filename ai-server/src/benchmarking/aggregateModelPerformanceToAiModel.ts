...
export async function aggregateModelPerformanceToAiModel(modelId: string, allModelPerformanceNodes: Node[]): Promise<void> {
    ...
    for (const node of allModelPerformanceNodes) {
        ...
        // Only aggregate the most recent 20 scores per node
        const cappedScoresArr = Array.isArray(node.scores) && node.scores.length > 20 ? node.scores.slice(-20) : node.scores;
        if (cappedScoresArr) {
            for (const scoreEntry of cappedScoresArr) {
                const qualityResults = scoreEntry.qualityResults as Partial<Record<BenchmarkType, QualityBenchmarkScore[]>>;
