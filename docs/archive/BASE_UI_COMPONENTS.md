...
The VerseForge frontend features a comprehensive design system with modular UI components that serve as the foundation for all user interfaces. These components are designed with plugin extensibility, universe theming, accessibility, and performance in mind.
...
```tsx
import { Button } from '@verseforge/ui-core';
